...
static char	test_ft_list_push_front_case(char const *test_name, t_list *begin_list, t_args *args)
{
	char	ko;

	if (args->verbose > 1)
		printf("\tft_list_push_front(begin_list):");
	else if (args->verbose)
		printf("\t%s", test_name);
	ko = FALSE;
	if (ft_list_push_front(begin_list) != ft_list_push_front_libft(begin_list))
		ko = TRUE;
	if (args->verbose)
		print_result(ko);
	return (ko);
...
void	test_ft_list_push_front_cases(char *ko, t_args *args)
{
	...
	begin_list = NULL;
	*ko = test_ft_list_push_front_case("empty_list:\t", begin_list, args);
	ft_list_push_front_libft(&begin_list, NULL);
	if (test_ft_list_push_front_case("1_link_list:\t", begin_list, args))
		*ko = TRUE;
	i = 0;
	while (i++ < 10)
		ft_list_push_front_libft(&begin_list, NULL);
	if (test_ft_list_push_front_case("small_list:\t", begin_list, args))
		*ko = TRUE;
	while (i++ < 100)
		ft_list_push_front_libft(&begin_list, NULL);
	if (test_ft_list_push_front_case("big_list:\t", begin_list, args))
		*ko = TRUE;
