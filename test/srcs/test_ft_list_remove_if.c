...
/*   Updated: 2021/01/20 07:38:25 by clbrunet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	test_ft_list_remove_if_case(char const *test_name, t_list *begin_list, void *data_ref, t_args *args)
{
	...
	if (begin_list_backup && !begin_list)
		ko = TRUE;
	else if (check_list_removals(begin_list, data_ref))
		ko = TRUE;
	if (ko == FALSE)
...
static void	test_ft_list_remove_if_noremove_cases(char *ko, t_args *args)
{
	...
	if (test_ft_list_remove_if_case("1_link_list_noremove:\t", begin_list, ":", args))
		*ko = TRUE;
	begin_list = NULL;
	i = 0;
	while (i++ < 10)
...
static void	test_ft_list_remove_if_remove_cases(char *ko, t_args *args)
{
	...
	begin_list = ft_lstnew(strdup("."));
	if (test_ft_list_remove_if_case("1_link_list_remove:\t", begin_list, ".", args))
		*ko = TRUE;
	begin_list = NULL;
	ft_list_push_front_libft(&begin_list, strdup("."));
	i = 0;
